 ...
 */
function normalise(input, output, { trimSec, encoder, log = () => undefined }) {
...
export function fetchBundle(workDir, name, { log = () => undefined, force = false } = {}) {
	...
	if (force || !existsSync(screen)) {
		normalise(rawScreen, screen, { trimSec: spec.trimSec, encoder, log });
	}
	...
	if (spec.webcam) {
		...
		if (force || !existsSync(webcam)) {
			normalise(rawCam, webcam, { trimSec: spec.trimSec, encoder, log });
		}
	}
