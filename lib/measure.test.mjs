...
describe("audioDurationReason", () => {
	it("is null when the track matches the source within tolerance", () => {
		expect(audioDurationReason({ durationSec: 60.05 }, 60.007, 0.75)).toBeNull();
	});

	it("flags a track carrying half the recording at matching loudness", () => {
		expect(audioDurationReason({ durationSec: 30.02 }, 60.007, 0.75)).toMatch(/30\.02/);
	});

	it("is null rather than a verdict when the duration is unknown", () => {
		expect(audioDurationReason({ durationSec: null }, 60.007, 0.75)).toBeNull();
		expect(audioDurationReason(null, 60.007, 0.75)).toBeNull();
	});
});
...
describe("waitForStableFile", () => {
	...
	it("uses the last-write time when the file completed before watching began", async () => {
		dir = mkdtempSync(join(tmpdir(), "stable-file-"));
		const file = join(dir, "output.mp4");
		writeFileSync(file, Buffer.alloc(8192));
		const writtenAt = new Date(Date.now() - 500);
		utimesSync(file, writtenAt, writtenAt);

		const watchedAt = now();
		const result = await waitForStableFile(file, { stableMs: 20, pollMs: 5 });

		expect(result.ok).toBe(true);
		expect(watchedAt - result.completedAt).toBeGreaterThan(350);
		expect(watchedAt - result.completedAt).toBeLessThan(700);
	});
});
