...
export function instantaneousCpuFor(matchPrefixes) {
	...
	try {
		...
		const raw = powershell(
			// Counter *paths* are localised — "\\Process(x)\\% Processor Time" does not resolve on a
			// non-English Windows, so this returned 0 and the foreign-load subtraction below became
			// a no-op. WMI class and property names are invariant. The #1/#2 suffix disambiguates
			// same-named processes and is not part of the image name.
			`$n = @(${names.map((n) => `'${n.replace(/'/g, "''")}'`).join(",")})
			 $s = 0
			 foreach ($p in (Get-CimInstance Win32_PerfFormattedData_PerfProc_Process)) {
			   if ($n -contains ($p.Name -replace '#\\d+$','')) { $s += $p.PercentProcessorTime }
			 }
			 $s`,
			{ timeoutMs: 20_000 },
		);
