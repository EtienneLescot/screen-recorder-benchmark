...
/**
 * The part of PowerShell's stderr that says what went wrong.
 *
 * PowerShell serialises stderr as CLIXML, and the first thing it writes is a run of progress
 * records — "Preparing modules for first use" — several hundred characters long. Truncating
 * raw stderr therefore reports the progress bar and drops the exception, which is how a
 * vanished UI element came back as an unreadable wall of XML on every GUI adapter.
 */
function psError(stderr) {
	const raw = String(stderr ?? "");
	const messages = [...raw.matchAll(/<S S="Error">([^<]*)<\/S>/g)].map((m) =>
		m[1].replace(/_x000D__x000A_/g, " ").trim(),
	);
	const text = messages.length ? messages.join(" ") : raw.replace(/<[^>]+>/g, " ");
	return text.replace(/\s+/g, " ").trim().slice(0, 600) || "(no error text)";
}
...
${script}`,
	...
	if (res.status !== 0) {
		throw new Error(`powershell exited ${res.status}: ${psError(res.stderr)}`);
	}
...
export function machineFingerprint() {
	return tidyStrings(probeMachine());
}

/**
 * Whitespace out of the fingerprint, once, where it enters the record.
 *
 * WMI pads `Win32_Processor.Name` to a fixed width, so this machine recorded its chip as
 * "AMD Ryzen 5 7520U with Radeon Graphics         " under Windows and the same string trimmed
 * under Linux — one laptop under two spellings, counted as two machines by everything downstream
 * that groups on the name. Two readers had already worked around it locally, each seeing only its
 * own symptom. Trimming at the probe is the version of that fix the next reader does not have to
 * write again.
 */
function tidyStrings(o) {
	const tidy = (v) => (typeof v === "string" ? v.replace(/\s+/g, " ").trim() : v);
	for (const [k, v] of Object.entries(o)) o[k] = Array.isArray(v) ? v.map(tidy) : tidy(v);
	return o;
}
...
/**
 * Two readings of a kernel counter, a known interval apart. The only way to get a figure that
 * describes *now* rather than a process's whole life.
 *
 * `ps -o pcpu` does not do this. It reports CPU time over the process's entire lifetime, so a
 * seven-hour-old desktop app contributes its seven-hour average no matter what it is doing
 * during the export being measured, and a freshly spawned ffmpeg contributes ~100 % because it
 * has never done anything else. Summed, that is not a load reading at all — measured here, it
 * quantised into two buckets, 37 % and 136 %, with nothing in between, and which bucket a leg
 * landed in decided a submission's weight through `loadSpreadPercentagePoints`. One sample
 * flipping moved a tool's median by a hundred points and halved the submission.
 *
 * /proc is authoritative and costs nothing to read twice.
 */
const LINUX_SAMPLE_MS = 250;
/** USER_HZ. Effectively always 100 on Linux, and what /proc/[pid]/stat counts in. */
const USER_HZ = 100;

/** Idle and total jiffies from the aggregate line of /proc/stat. */
function linuxCpuTotals() {
	const first = (readFile("/proc/stat") ?? "").split("\n")[0] ?? "";
...
/** Pids whose full command line contains any of the prefixes. */
function linuxPidsMatching(prefixes) {
	const pids = [];
	try {
		for (const entry of readdirSync("/proc")) {
			if (!/^\d+$/.test(entry)) continue;
			const raw = readFile(`/proc/${entry}/cmdline`);
			if (!raw) continue; // kernel threads have an empty cmdline
			const args = raw.replace(/\0/g, " ").trim();
			if (prefixes.some((p) => args.includes(p))) pids.push(entry);
		}
	} catch {
		/* /proc unreadable: a container with a restricted view */
	}
	return pids;
}

/**
 * utime + stime out of one /proc/[pid]/stat line, in jiffies. Exported for the unit tests.
 *
 * The comm field is parenthesised and may contain spaces *and* parentheses — a process can be
 * called `(my app) v2` — so splitting the line on whitespace puts every later field at an
 * offset that depends on the process's name. Counting from the *last* ')' is the documented way
 * to parse this, and it is the whole reason this is a named function rather than two lines
 * inline: get it wrong and the numbers are silently plausible.
 */
export function parseProcStatJiffies(line) {
	const close = String(line).lastIndexOf(")");
	if (close < 0) return null;
	// Fields after comm, with state at index 0: utime is field 14, stime 15, so 11 and 12 here.
	const after = line.slice(close + 2).split(/\s+/);
...
/** Summed utime+stime jiffies for the given pids. */
function linuxPidJiffies(pids) {
	let total = 0;
	for (const pid of pids) {
		const st = readFile(`/proc/${pid}/stat`);
		if (!st) continue;
		total += parseProcStatJiffies(st) ?? 0;
	}
	return total;
}

/** Sleep synchronously, because every caller of these is synchronous. */
const sleepSync = (ms) => spawnSync("/bin/sleep", [String(ms / 1000)]);
...
export function instantaneousCpuFor(matchPrefixes) {
	...
	if (IS_LINUX) {
		try {
			// The pid set is taken once and reused for both readings: a process that appears or
			// exits mid-interval has no before-or-after pair and cannot contribute a delta.
			const pids = linuxPidsMatching(prefixes);
			if (!pids.length) return 0;
			const before = linuxPidJiffies(pids);
			const t0 = Date.now();
			sleepSync(LINUX_SAMPLE_MS);
			const elapsed = (Date.now() - t0) / 1000;
			const after = linuxPidJiffies(pids);
			if (elapsed <= 0) return 0;
			return +(((after - before) / USER_HZ / elapsed) * 100).toFixed(1);
		} catch {
			return 0;
		}
	}
...
export function instantaneousLoadPercent() {
	if (IS_LINUX) {
		try {
			const before = linuxCpuTotals();
			if (!before) return null;
			sleepSync(LINUX_SAMPLE_MS);
			const after = linuxCpuTotals();
			if (!after) return null;
			const total = after.total - before.total;
			const idle = after.idle - before.idle;
			if (total <= 0) return null;
			// Scaled to the convention the other platforms report in: percentages summed across
			// cores, so 100 % is one core saturated and 800 % is an eight-thread part flat out.
			return Math.round(((total - idle) / total) * os.cpus().length * 100);
		} catch {
			return null;
		}
	}
 ...
 * looks like two builds: the aggregate warned "openscreen spans 2 versions (1.10.0,
...
export function appVersion(appPath) {
	...
	try {
		// FileVersion before ProductVersion: electron-builder writes ProductVersion as the
		// four-component numeric field, which cannot hold a prerelease suffix — a 1.11.0-rc.1
		// build reports "1.11.0.0" there and was recorded as the shipped 1.11.0, merging a
		// release candidate into the very build it was meant to be measured against. FileVersion
		// carries the real string, matching what macOS reads out of Info.plist and Linux out of
		// the asar — one release reporting one version on all three, which is the stated point.
		return (
			trimBuildField(
				powershell(
					`(Get-Item ${JSON.stringify(appPath)}).VersionInfo | ` +
						"ForEach-Object { if ($_.FileVersion) { $_.FileVersion } else { $_.ProductVersion } }",
					{ timeoutMs: 15_000 },
				),
			) || null
		);
