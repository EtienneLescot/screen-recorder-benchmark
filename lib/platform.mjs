	...
	try {
		...
		const raw = powershell(
			`$n = @(${names.map((n) => `'${n.replace(/'/g, "''")}'`).join(",")})
			 $s = 0
			 foreach ($p in (Get-Process | Where-Object { $n -contains $_.ProcessName })) {
			   try { $s += (Get-Counter "\\Process($($_.ProcessName))\\% Processor Time" -ErrorAction Stop).CounterSamples[0].CookedValue } catch {}
			 }
			 $s`,
			{ timeoutMs: 20_000 },
		);
