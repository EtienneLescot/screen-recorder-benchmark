...
export function buildWebcam(workDir, spec) {
	const { ffmpeg } = resolveFfmpeg();
	// Frames from main memory: the whole clip is a lavfi source graph passed as `-i`, so there is
	// no output filter chain to end with an upload. Generating the camera track is not measured.
	const enc = pickH264Encoder(ffmpeg, { allowHwFrames: false });
	const dir = join(workDir, "fixture");
	mkdirSync(dir, { recursive: true });
...
/**
 * FocuSee keeps its pointer beside the footage, inside the `.focusee` project package:
 * `recording/mousemoves.json` and `recording/mouseclicks.json`, in **screen pixels** against the
 * display session's `bounds`, stamped on the same `processTimeMs` clock as `recording/
 * metadata.json`, plus a `cursors.json` table naming the shapes that appear.
 *
 * An import writes all three as empty arrays — FocuSee has no telemetry for a file it did not
 * record — so a file-sourced project renders no pointer at all until this puts one there. Every
 * field below is copied from a project FocuSee recorded itself; guessing the names would fail
 * silently, exactly as it did for Cap, because the app parses the file and simply finds nothing
 * it recognises.
 */
export function writeFocuseeCursor(projectDir, spec) {
	return writeFocuseeTelemetry(projectDir, cursorTrack(spec));
}

/** The same, from a real recording's OpenScreen sidecar rather than the generated track. */
export function convertCursorForFocusee(openscreenSidecar, projectDir) {
	const doc = JSON.parse(readFileSync(openscreenSidecar, "utf8"));
	return writeFocuseeTelemetry(projectDir, doc.samples ?? []);
}

function writeFocuseeTelemetry(projectDir, samples) {
	const rec = join(projectDir, "recording");
	const meta = JSON.parse(readFileSync(join(rec, "metadata.json"), "utf8"));
	const display = meta.recorders?.find((r) => r.type === "display")?.sessions?.[0];
	if (!display?.bounds?.width) {
		throw new Error(`${projectDir} has no display session to align a pointer track against`);
	}
	// The clock is the project's own: samples that do not overlap the display session are
	// outside the clip and are drawn nowhere.
	const t0 = display.processTimeStartMs;
	const { width, height } = display.bounds;
	const at = (s) => ({
		processTimeMs: t0 + s.timeMs,
		cursorId: "arrow",
		x: Math.round(s.cx * width),
		y: Math.round(s.cy * height),
	});
	const moves = samples.map((s) => ({ ...at(s), type: "mouseMoved" }));
	const clicks = samples
		.filter((s) => s.interactionType && s.interactionType !== "move")
		.map((s) => ({
			...at(s),
	...
	writeFileSync(join(rec, "mousemoves.json"), `${JSON.stringify(moves, null, 0)}\n`);
	writeFileSync(join(rec, "mouseclicks.json"), `${JSON.stringify(clicks, null, 0)}\n`);
	// `systemCursor` tells FocuSee to draw its own themed arrow rather than a bitmap from the
	// recording, which is what the scenario asks for: the app's pointer, not the source's.
	writeFileSync(
		join(rec, "cursors.json"),
		`${JSON.stringify(
			[
				{
					id: "arrow",
					systemCursor: true,
					standardSize: { width: 28, height: 40 },
					hotSpot: { x: 5, y: 5 },
					hash: "",
				},
			],
