...
export function buildSubmission(doc, { submitter = undefined } = {}) {
	...
	const open = floorMedian(openFloor);
	...
	return {
		...
		conditions: {
			...
			loadSpreadPercentagePoints: allLoads.length
				? +(Math.max(...allLoads) - Math.min(...allLoads)).toFixed(1)
				: null,
			driftRatio: open && close ? +(close / open).toFixed(4) : null,
			// Not detectable with certainty; the submitter is expected to set it honestly, and
			// the protocol says why it matters more than the CPU figure suggests.
	...
	const walk = (d) => {
		for (const entry of readdirSync(d)) {
			...
			if (statSync(p).isDirectory()) walk(p);
				...
				try {
					out.push({ ...JSON.parse(readFileSync(p, "utf8")), _path: p });
				} catch {
...
export function renderAggregate(result, { step, submissions } = {}) {
	...
	if (result.consistency && !result.consistency.measurable) {
		...
		L.push(
			`consistency: redundant paths disagree by ${result.consistency.medianResidualPercent}% (median), ` +
				`${result.consistency.maxResidualPercent}% at worst` +
				(result.consistency.worst
					? ` — ${result.consistency.worst.a} vs ${result.consistency.worst.b} on ${result.consistency.worst.machine}`
					: ""),
		);
	}
