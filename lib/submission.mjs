...
export function renderAggregate(result, { step, submissions, basis = "hardware" } = {}) {
	const L = [];
	L.push(
		`Aggregate${step ? ` — ${step}` : ""}${basis === "software" ? " — against the software floor (libx264)" : ""}: ` +
			`${submissions} submission(s), ${result.consistency?.observations ?? 0} measurement(s)`,
	);
	if (basis === "software") {
		L.push(
			"  The cores rather than the fixed-function block. Read beside the figures above, not " +
				"instead of them: a part whose encoder has more than one sustained clock moves every " +
				"hardware-floor cost without the exports moving at all.",
		);
	}
	if (result.consistency && !result.consistency.measurable) {
		L.push(
			"spread: not measurable — no build has been measured twice yet. A second run of any tool " +
				"already here would show how far its cost moves.",
		);
	} else if (result.consistency?.medianSpreadPercent != null) {
		const c = result.consistency;
		const bp = c.byPlatform ?? {};
		L.push(
			`spread: a build's cost moves by ${c.medianSpreadPercent}% at the median between runs ` +
				`(over ${c.repeatedBuilds} build(s) measured more than once), ${c.maxSpreadPercent}% at worst.`,
		);
		...
		if (bp.sameGpuRepeats) {
			L.push(
				`  repeated on the same platform and GPU it moves by ${bp.sameGpuSpreadPercent}% ` +
					`(median over ${bp.sameGpuRepeats} set(s)) — this is the figure that should be small.`,
			);
		} else {
			L.push(
				"  no build has been measured twice on one platform *and* GPU yet, so nothing here says " +
		...
		if (bp.samePlatformRepeats && bp.samePlatformSpreadPercent !== bp.sameGpuSpreadPercent) {
			L.push(
				`  on the same platform across different GPUs it moves by ${bp.samePlatformSpreadPercent}% ` +
					`(median over ${bp.samePlatformRepeats} set(s)) — the floor divides out the encoder, not ` +
					"the shader array the compositing actually runs on.",
			);
		}
		if (bp.crossPlatformRepeats) {
			L.push(
				`  between platforms it moves by ${bp.crossPlatformSpreadPercent}% (median over ` +
					`${bp.crossPlatformRepeats} build(s)) — expected, since a tool tuned for one media stack ` +
					"need not cost the same on another.",
			);
		}
		// Both ends, never one machine: the build moved between two runs and both are real.
		const w = c.worst;
		if (w?.ends?.length === 2) {
			const [lo, hi] = w.ends;
			L.push(
				`  widest: ${w.tool} — ${lo.cost}× on ${lo.machine} (${lo.platform}) against ` +
					`${hi.cost}× on ${hi.machine} (${hi.platform}).`,
			);
		}
	}
