...
export function buildSubmission(doc, { submitter = undefined } = {}) {
	...
	const measurements = rows
		.map((r) => {
			...
			return {
				...
				// How far the denominator moved *while this tool was being measured*. `localFloorMs`
				// is the median of the floors paired with this leg's scoring runs, so this says how
				// much those disagreed with each other — the error bar on the number this leg's
				// costs were divided by. Recorded here; nothing reads it yet.
				floorSpreadPercent: (() => {
	...
	return {
		...
		conditions: {
			...
			// The widest any single leg's paired floors disagreed with each other.
			floorSpreadPercent: (() => {
				const sp = measurements.map((x) => x.floorSpreadPercent).filter((x) => x != null);
				return sp.length ? +Math.max(...sp).toFixed(2) : null;
			})(),
