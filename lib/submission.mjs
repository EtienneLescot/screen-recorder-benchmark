...
export function renderAggregate(result, { step, submissions } = {}) {
	...
	if (result.consistency && !result.consistency.measurable) {
		...
		// Same-platform first: it is the one that would mean the measurement is wrong.
		if (bp.samePlatformPairs) {
			L.push(
				`consistency: runs on the same platform disagree by ${bp.samePlatformSpreadPercent}% (median over ` +
					`${bp.samePlatformPairs} pair(s)) — this is the figure that should be small.`,
			);
		} else {
			L.push(
