...
export function verifyOutput(path, target, sourceDurationSec, { sourceAudioDb = null } = {}) {
	...
	if (sourceAudioDb != null) {
		if (!p.audio) {
			...
			const db = meanVolumeDb(path);
			if (db == null) reasons.push("output audio could not be measured");
			// -60 dBFS is far below anything audible; digital silence measures about -91.
			else if (db < -60) reasons.push(`output audio is silent (mean ${db} dBFS)`);
			else if (db - sourceAudioDb < -12) {
				reasons.push(
					`output audio is ${(sourceAudioDb - db).toFixed(1)} dB quieter than the source`,
				);
			}
		}
	}
