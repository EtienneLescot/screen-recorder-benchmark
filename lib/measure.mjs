...
export async function waitForStableFile(
	...
	while (now() - t0 < timeoutMs) {
		...
		if (size > lastSize) {
			lastSize = size;
			lastGrowthAt = now();
			// mtime is wall-clock time while the benchmark stopwatch is monotonic. Translate it at
			// observation time so a driver that returns after the file has already finished does not
			// charge that delay to the export. Clamp future/skewed mtimes to the observation itself.
			lastWriteAt =
				mtimeMs == null ? lastGrowthAt : lastGrowthAt - Math.max(0, Date.now() - mtimeMs);
		}
		...
		if (lastSize >= minBytes && lastGrowthAt !== null && now() - lastGrowthAt >= stableMs) {
			return {
				ok: true,
				appearedAt,
				completedAt: lastWriteAt, // the filesystem's timestamp for the last byte
				sizeBytes: lastSize,
				waitedMs: now() - t0,
			};
		}
...
/**
 * The audio track must run as long as the source, not just make noise. FocuSee 2.3.5
 * exported 30s of sound in a 60s file at matching loudness — every loudness check
 * passed while half the recording's sound was missing. Null when the duration is
 * unknown (some containers report none), never a pass on missing data.
 */
export function audioDurationReason(audioProbe, sourceDurationSec, toleranceSec) {
	const d = audioProbe?.durationSec;
	if (d == null || sourceDurationSec == null) return null;
	return Math.abs(d - sourceDurationSec) > toleranceSec
		? `audio runs ${d.toFixed(2)}s vs source ${sourceDurationSec}s (Δ${Math.abs(d - sourceDurationSec).toFixed(2)}s)`
		: null;
}
...
export function verifyOutput(path, target, sourceDurationSec, { sourceAudioDb = null } = {}) {
	...
	if (sourceAudioDb != null) {
		if (!p.audio) {
			...
			const tooShort = audioDurationReason(
				p.audio,
				sourceDurationSec,
				target.tolerance.durationSec,
			);
			if (tooShort) reasons.push(tooShort);
		}
	}
