 ...
 * warns that the padding solve is stale when it is not, and `saveCalibration` drops every other
 * app's entry as though it belonged to someone else's hardware.
 */
...
export function saveCalibration(entries, meta) {
	...
	const doc = {
		...
		...meta,
		// Merged, not replaced — but only over a solve from this same machine.
		//
		...
		//
		// Carrying entries across a *different* machine would be worse than dropping them, so the
		// stamp gates it: a macOS solve is not kept alive by a Windows one. That is also why this
		// merges rather than the file being per-machine — the shape stays as it is, and `run`
		// already warns when the stamp does not match.
		apps: {
			...(sameMachine(previous.machine, m) ? (previous.apps ?? {}) : {}),
	...
	// otherwise touched.
	writeFileSync(CALIBRATION_PATH, `${JSON.stringify(doc, null, "\t")}\n`);
