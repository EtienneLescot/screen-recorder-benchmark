 ...
 * keeps its own figure and is published on the builds page.
 */
...
export function aggregate(submissions, { step = null, basis = "hardware" } = {}) {
	...
	const perSetup = (obs) => {
		...
		for (const o of obs) {
			...
			by.get(k).push({ value: o.cost, weight: o.weight });
		}
		return [...by.values()].map(weightedMean);
	};
	const tools = [...nodes].map((node) => {
		...
		return {
			...
			relativeCost: +(
				perSetup(obs).reduce((n, x) => n + x, 0) / new Set(obs.map(setupKey)).size
			).toFixed(3),
			setups: new Set(obs.map(setupKey)).size,
