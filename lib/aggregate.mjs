...
/**
 * The build a measurement belongs to — the node it becomes in the graph.
 *
...
/** The node a tool's build occupies in the graph. */
export const nodeId = (tool, build) => `${tool}@${build}`;

/**
 * The newest build of each tool in a solved result, which is what the headline ranks.
 *
 ...
 * first submission on it. A build that is in the tree but not the newest is not deleted — it
 * keeps its edges, holds the graph together, and is published on the builds page.
 */
export function newestBuilds(tools) {
	const best = new Map();
	for (const t of tools) {
		const cur = best.get(t.tool);
		if (!cur || compareBuilds(t.build, cur.build) > 0) best.set(t.tool, t);
	}
	return tools.filter((t) => best.get(t.tool) === t);
}
...
export function aggregate(submissions, { step = null, basis = "hardware" } = {}) {
	...
	for (const sub of submissions) {
		...
		const priced = (sub.measurements ?? [])
			.map((m) => ({ m, cost: normalisedCost(m, basis), build: buildKey(m.version) }))
		...
		if (costed.length < 2) {
			skipped.push({
				...
				why:
					priced.length >= 2
						? "fewer than two measurements record which build they measured"
						: costed.length === 0
		...
		for (const { m, cost, node, build } of costed) {
			...
			perTool
				.get(node)
				.push({ cost, weight, machine: sub.machine, tool: m.tool, build, version: m.version, sub });
		}
	...
	const compOf = (node) => components.findIndex((c) => c.includes(node));
	const tools = [...nodes].map((node) => {
		...
		return {
			...
			component: compOf(node),
			relativeCost: Math.exp(logCost.get(node)),
	...
	for (const ci of components.keys()) {
		...
		for (const t of inComp) {
			for (const o of perTool.get(t.node) ?? []) {
	...
	return { tools, edges, used, skipped, components, consistency, buildSpread, unplaceable };
}
