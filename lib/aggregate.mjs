...
export function submissionWeight(sub) {
	...
	// How much the denominator moved *while each tool was measured*, not across the whole run.
	//
	// Costs are divided by the median of the floors paired with that leg's own scoring runs, so
	// a machine that settles between one leg and the next is already corrected: the denominator
	// ages with the numerator by construction. Weighting on the opening-vs-closing drift ratio
	// therefore charged a submission for an error the pairing had removed, and it charged it
	// against run length — which grows with the number of tools measured, so the submissions
	// carrying the most edges paid the most.
	//
	// It also charged it against machine class. Of the five submissions published before this
	// changed, the two desktops and the Apple part weighted ×1 and the only thermally-limited
	// laptop weighted ×0.6 and ×0.25, drift being the sole reason in every case. A benchmark
	// asserting that cost-in-units-of-the-floor travels between machines cannot afford to
	// down-rank the machines that test the assertion hardest.
	//
	// Measured on the run that prompted this: 19.9% drift across the run, 0.3-4.3% within each
	// leg, and 0.8% on the *software* floor over the same 90 minutes. The cores never wavered;
	// the fixed-function encoder settled to its sustained clock. That is one subsystem finding
	// its level, not a machine changing underneath a measurement, and the run's per-tool spreads
	// (0.56-0.67%) say so.
	//
	// `driftRatio` is still recorded and still published — it describes the machine over the run,
	// which is worth knowing. It is just not the error bar on the ratio.
	if (c.floorSpreadPercent != null) {
		if (c.floorSpreadPercent > 15) {
			w *= 0.25;
			reasons.push(
				`the floor moved ${c.floorSpreadPercent.toFixed(1)}% within a single leg — ` +
					"its exports were not divided by a stable unit",
			);
		} else if (c.floorSpreadPercent > 8) {
			w *= 0.6;
			reasons.push(`the floor moved ${c.floorSpreadPercent.toFixed(1)}% within a single leg`);
		}
	} else if (c.driftRatio != null) {
		// Submissions published before floorSpreadPercent existed cannot report it — it needs the
		// per-repetition paired floors, which they never carried. They keep the old rule rather
		// than being silently re-weighted on evidence nobody has.
