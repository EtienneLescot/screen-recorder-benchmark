 ...
 */
export async function fileDialogTo(processName, absolutePath, { stemOnly = true, timeoutMs } = {}) {
	// timeoutMs has to reach the Windows side: a dialog that only opens once a render has
	// finished is minutes away, not the 30s default, and the caller is the only one that knows.
	if (IS_WIN) return win.fileDialogTo(processName, absolutePath, timeoutMs ? { timeoutMs } : {});
