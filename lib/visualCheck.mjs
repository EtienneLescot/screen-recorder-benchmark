...
export function inspectExport(
	...
	if (e.background?.kind === "image") {
		...
		// A background was applied if all four corners agree with each other and none of them is
		// the recording.
		//
		// This deliberately does not require the scenario's own wallpaper, or even a light one.
		// Handing a custom image to every app is awkward — some restrict which paths they will
		// read — so a tool's own default backdrop is accepted, which is the roster's decision and
		// not this file's to relitigate. What still has to be true is that *something* was
		// composited behind the recording: four mutually consistent corners that differ from the
		// content is that, and it holds for a dark backdrop as readily as a light one. The
		// earlier `> 110` test only ever passed light wallpapers, which would have failed a dark
		// default for no reason connected to the work being measured.
		const cornerLum = [
		...
		const contentLum = luminance(
			px(img, Math.round(box.left + box.width / 2), Math.round(box.top + box.height / 2)),
		);
		result.measured.contentLuminance = Math.round(contentLum);
		const cornersAgree = Math.max(...cornerLum) - Math.min(...cornerLum) < 60;
		const differsFromContent = Math.abs(cornerLum[0] - contentLum) > 25;
		result.checks.background = cornersAgree && differsFromContent;
