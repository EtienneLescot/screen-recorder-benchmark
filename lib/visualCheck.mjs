 ...
 * work at all.
 *
 * Each edge is walked inward and the box starts at a step — a short run of pixels far from the
 * pixel `lag` before them, which is still background until the edge is crossed.
 *
 * The step is what matters, not the distance from a fixed reference. Comparing every pixel to
 * the frame's outermost pixel on that row — the previous approach — assumes the wallpaper is
 * flat, and silently fails when it is not. OpenScreen composites its own dark red gradient: its
 * left edge reads [90,20,45] against a [22,26,34] editor, 85 apart under a tolerance of 90, so
 * the scan crossed the entire recording without seeing it and returned a box one pixel wide.
 * The gradient's own drift across that span was 78 — twice the 37 of the content step it was
 * supposed to detect — so no absolute threshold against a fixed reference can separate them.
 * Measured against the pixel 8 back, background steps stay under 21 and the content edge is 40
 * or more, on every row of the same frame.
 *
 * Each edge is read on several lines through the middle half of the frame and the median taken,
 * so one unlucky row across a textured wallpaper cannot move the box on its own.
 */
export function contentBoxAgainstBorder(img, { tolerance = 30, run = 4, lag = 8, lines = 9 } = {}) {
	const midY = Math.floor(img.height / 2);
	const midX = Math.floor(img.width / 2);
	// One line walked inward from its border, parameterised by distance from that border so all
	// four edges share the logic. Returns null when no step is found at all.
	const edgeAt = (sample, len) => {
		const step = (t) => (t < lag || t >= len ? 0 : dist(sample(t), sample(t - lag)));
		let first = -1;
		for (let t = lag; t < len - run && first < 0; t++) {
			let all = true;
			for (let k = 0; k < run && all; k++) if (step(t + k) <= tolerance) all = false;
			if (all) first = t;
		}
		if (first < 0) return null;

		// A drop shadow ramps into the recording over a dozen pixels, and its own steps clear the
		// threshold before the edge does: Cap's shadow reaches 64 where its content edge is 542.
		// Where a far stronger step follows within a shadow's width, that step is the recording
		// and the first crossing was its shadow. This is what keeps the corner-radius probes
		// honest — landing in the shadow puts both of them on the same ramp, which is how a box
		// five pixels out scored 94 against a threshold of 90 and passed by accident.
		let peak = 0;
		for (let t = first; t < first + 3 * lag; t++) peak = Math.max(peak, step(t));
		if (peak > 3 * step(first)) {
			for (let t = first; t < first + 3 * lag; t++) if (step(t) > peak / 2) return t;
		}
		return first;
	};

	// Lines through the middle half only: the padding under test is a small fraction of the
	// frame, so every one of them crosses the recording, while a line near an edge need not.
	const spread = (len) =>
		Array.from({ length: lines }, (_, i) =>
			Math.floor(len / 4 + (len / 2) * ((i + 1) / (lines + 1))),
		);
	const median = (a) => a.slice().sort((p, q) => p - q)[a.length >> 1];

	// Finding no step means the recording reaches that border, which is a real answer — zero
	// inset — and has to be reported as one. The previous scan returned its loop bound instead,
	// so a full-frame export read as a 1916px inset and passed the padding check it should have
	// failed.
	const ys = spread(img.height);
	const xs = spread(img.width);
	const W = img.width;
	const H = img.height;
	const left = median(ys.map((y) => edgeAt((t) => px(img, t, y), W) ?? 0));
	const right = median(ys.map((y) => W - 1 - (edgeAt((t) => px(img, W - 1 - t, y), W) ?? 0)));
	const top = median(xs.map((x) => edgeAt((t) => px(img, x, t), H) ?? 0));
	const bottom = median(xs.map((x) => H - 1 - (edgeAt((t) => px(img, x, H - 1 - t), H) ?? 0)));
	...
	return {
		...
		border: {
			left: px(img, 0, midY),
			right: px(img, img.width - 1, midY),
			top: px(img, midX, 0),
			bottom: px(img, midX, img.height - 1),
		},
	};
}
...
export function inspectExport(
	...
	if (e.background?.kind === "image") {
		...
		// Either the border varies like an image, or the recording sits inside a consistent fill.
		//
		// Variation alone was the first attempt and it fails on a real export: Cap's default
		// backdrop is flat white, so `borderVariation` reads near zero and a frame with a plainly
		// composited background — inset, rounded, shadowed — was reported as having none. Now
		// that the scenario accepts each tool's own backdrop, it has to accept a flat one, and a
		// default is as likely to be a solid colour as a photograph.
		//
		// The second test is the inset `contentBoxByLuminance` already computed. What separates
		// "something was composited behind the recording" from "the recording fills the frame" is
		// that margin, and it holds whatever colour either of them is.
		const variation = borderVariation(img);
		result.measured.borderVariation = Math.round(variation);
		const inset = box.left > 2 && box.top > 2 && box.right < W - 3 && box.bottom < H - 3;
		result.measured.backgroundInset = inset;
		result.checks.background = variation > 30 || inset;
		...
		if (e.cornerRadiusPx > 0 && box.width > 40) {
			...
			// top edge it must show the recording. Both are judged against the border beside the
			// corner, not against a fixed luminance — and beside the corner is the whole point.
			// The first version compared both probes to the frame's mid-top pixel, which assumes
			// the backdrop is one flat colour: FocuSee's own wallpaper drifts 416 over 50px, so a
			// plainly rounded corner read 138 away from a reference taken 900px to its right, and
			// the check failed on the only export that had exposed it. A reference sampled
			// 15-20px outside the corner carries the same wallpaper gradient and the same drop
			// shadow ramp as the corner probe itself, so both cancel and only the geometry
			// remains. With no margin outside the box there is nowhere local to sample, and the
			// mid-top pixel is all that is left.
			const beside =
				box.left > 24 && box.top > 24
					? [
							px(img, box.left - 15, box.top - 15),
							px(img, box.left - 20, box.top + 1),
							px(img, box.left + 1, box.top - 20),
						]
					: null;
			const ref = beside
				? beside[0].map((_, c) => beside.map((p) => p[c]).sort((a, b) => a - b)[1])
				: box.border.top;
