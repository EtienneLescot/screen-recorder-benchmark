...
export function buildProject({
	...
	const editor = {
		// KNOWN BROKEN — this is what stops the adapter working, and it is here, not in the app.
		//
		// `wallpaper` is not a path. BUILT_IN_WALLPAPERS in src/lib/wallpapers.ts holds bare
		// filenames — "tahoe-light.jpg" — which the app joins onto its own asset base. Handed an
		// absolute Windows path it produces
		//   C:\C%3A%5CUsers%5Cetien%5C...%5Cide-1080p60-60s.wallpaper.png
		// — its base, then the whole path percent-encoded as a single segment — and logs
		// "Failed to read local file". The background is silently missing from the render, and on
		// the second read attempt the network service crashes, which is what leaves the export
		// dying on "VideoEncoder is not defined".
		//
		// The fix is to hand it whatever a custom import produces (see "Importer un fond
		// personnalisé" in the editor) rather than a filesystem path. Until then this stays, so
		// the failure is reproducible rather than papered over with a built-in wallpaper the
		// other tools are not compositing.
