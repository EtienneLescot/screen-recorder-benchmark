...
export function download(url, destDir, { log = () => undefined } = {}) {
	...
	if (!/\.(dmg|zip|pkg|exe|msi|appimage|deb|rpm)$/i.test(name))
		name = `${name || "download"}${IS_WIN ? ".exe" : IS_LINUX ? ".AppImage" : ".dmg"}`;
 ...
 * mount would have.
 */
export function installAppImage(imagePath, appName, { log = () => undefined } = {}) {
...
export function installApp(spec, { cacheDir, force = false, log = () => undefined } = {}) {
	...
	if (IS_WIN) {
		...
		if (!/\.appimage$/i.test(dl.path)) {
			throw new Error(
				`only .AppImage installs are automated on Linux; got ${basename(dl.path)}. ` +
					"A .deb or .rpm would need root, which this harness does not take.",
			);
		}
		installAppImage(dl.path, spec.appName, { log });
