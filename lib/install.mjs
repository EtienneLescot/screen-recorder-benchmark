 ...
 *
 * `tag` names one release instead of taking whatever is current. Two reasons it exists: a
 * measurement of "the latest build" cannot be repeated once the vendor ships another one, and
 * `/releases/latest` is not the newest release — GitHub excludes prereleases from it, so a
 * release candidate is unreachable without naming its tag. Measuring one is a legitimate thing
 * to want (does the rc regress against the release it replaces?), and every run records the
 * version it actually measured, so a pinned build is never silently folded in with the shipped
 * one.
 */
export function resolveGithubAsset(repo, pattern, { tag = null } = {}) {
	const endpoint = tag
	...
	try {
		...
		// curl -f reports an HTTP 404 as exit 22 and no body, which surfaced as a bare
		// "Command failed" with nothing naming the tag that does not exist.
		if (tag) throw new Error(`no release tagged ${tag} in ${repo} (${e.message.split("\n")[0]})`);
		throw e;
	}
...
export function installApp(spec, { cacheDir, force = false, log = () => undefined } = {}) {
	...
	if (spec.method === "github-release") {
		const asset = resolveGithubAsset(spec.repo, spec.assetPattern, { tag: spec.version });
		url = asset.url;
