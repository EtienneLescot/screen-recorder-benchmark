...
export function summarise(appResult) {
	...
	return {
		...
		localFloorPaired: appResult.localFloor?.paired ?? null,
		// The CPU-side floor measured beside the hardware one. Recorded and published since
		// 98f6024, but reading it meant opening results.json.
		softwareFloorMs: appResult.softwareFloor?.exportMs ?? null,
...
export function renderReport(doc) {
	...
	md.push("");
	// The closing control against the opening one. Computed since the run existed, and until now
	// never printed — which made a drifting machine indistinguishable from a steady one.
	if (drift != null) {
		const pct = Math.round(Math.abs(drift - 1) * 100);
		md.push(
			drift > 1.08 || drift < 0.92
				? `**⚠ Drift ${drift}×** — the identical floor workload took ${pct}% ${drift > 1 ? "longer" : "less"} at the end of the run than at the start. The machine changed underneath the measurements, so tools measured later carry a handicap. Re-run somewhere quieter before quoting these numbers.`
				: `**Drift ${drift}×** — the floor took the same time at the end of the run as at the start, within ${pct}%, so the ordering did not bias the results.`,
		);
	}
	md.push("");
	...
	if (loadSpread > 60) {
		md.push("");
		md.push(
			`**⚠ Background load varied by ${loadSpread} percentage points between apps.** They were not measured under the same conditions, so the raw times are not a ranking, and **Cost (×floor) does not repair it**: the floor runs on the fixed-function encoder block, which barely throttles and is largely indifferent to CPU contention, while the compositing under test runs on cores and shaders, which are not. Measured on one machine, background load doubled between two runs while the floor moved 1.1% and the export it was dividing moved 19%. Re-run somewhere quiet; there is no column here that corrects for this.`,
		);
		md.push("");
	}
	md.push(
	...
	md.push(
		"- **How it was driven** records the automation rung: `cli` is scripted headlessly, `cdp` drives an Electron app over its debugging port, `menu` is System Events driving named menu items, `menu+coords` means some step needed pixel clicking and is the least reproducible.",
		"- **Does the rung cost the tool anything?** Only one entrant is driven differently from the rest, and it is the slowest, so the question is worth answering with a number rather than an assurance. Everything the automation does to *set up* an export — launching, loading the project, pinning the export settings — happens before `ctx.commit()` and is not in the measured window at all; on Recordly that is around 185 s per repetition, none of it counted. What remains inside the window is the progress poll: one `document.body.innerText` read every 2 s, measured at 1 ms median over 12 samples, so about 14 ms across a 28.7 s render. That is 0.05% of the row, roughly a thousandth of the gap it would have to explain.",
	);
	md.push("");
	...
	for (const r of rows.filter((x) => x.notes?.length || x.failures?.length)) {
		...
		md.push("");
		// The two floors, side by side, because their ratio is the thing a reader needs in order to
		// know how far this row travels. The hardware floor is the fixed-function encoder block;
		// the software one is the same clip through libx264, bound by the cores instead. A machine
		// whose block is fast relative to its cores flatters every compositor divided by it.
		if (r.localFloorMs && r.softwareFloorMs) {
			md.push(
				`- floors beside this leg: ${(r.localFloorMs / 1000).toFixed(2)}s hardware` +
					`${r.localFloorPaired ? ` (median of ${r.localFloorPaired} paired with the scoring runs)` : ""}` +
					`, ${(r.softwareFloorMs / 1000).toFixed(2)}s libx264 — ` +
					`${(r.softwareFloorMs / r.localFloorMs).toFixed(2)}× apart. Cost is divided by the first; ` +
					"the second is what the cores were doing at the same moment.",
			);
		}
	...
	const os = ranked.find((r) => r.app === "openscreen");
