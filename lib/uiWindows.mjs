...
function Get-AppWindows([string]$proc) {
  ...
  # descendant walk stays bounded; measured at 1.2s against a running Electron app.
  $out = @()
  ...
  foreach ($p in (Get-Process -Name $proc -ErrorAction SilentlyContinue)) {
    $cPid = New-Object System.Windows.Automation.PropertyCondition(
      [System.Windows.Automation.AutomationElement]::ProcessIdProperty, $p.Id)
    $and = New-Object System.Windows.Automation.AndCondition($cPid, $cWin)
    $out += $root.FindAll([System.Windows.Automation.TreeScope]::Descendants, $and)
  }
  return $out
}
...
export async function fileDialogTo(
	...
	const t0 = Date.now();
	let ready = false;
	// Phase timings, returned to the caller. A GUI adapter's export clock runs across this
	// call, and without the split there is no way to tell the app finishing its render from
	// this function's own cost — one listWindows is a PowerShell spawn plus a UIA descendant
	// walk, measured at 1.2-1.5s each on this machine, and the commit step makes several more.
	let polls = 0;
	while (Date.now() - t0 < timeoutMs) {
		polls++;
	...
	const timings = () => ({ appearedMs, filledMs, committedMs: Date.now() - t0, polls });

	if (commit) {
		await sleep(400);
		// "Save" is tried first and misses on every localised build, so on this French machine
		// the hit is always the second walk. Each miss is another PowerShell round trip.
		...
		if (!r.ok) {
			...
			for (const label of ["Enregistrer", "Export", "OK", "Open", "Ouvrir"]) {
				if (clickControl(processName, label, { controlType: "Button" }).ok)
					return { path: absolutePath, file: basename(absolutePath), timings: timings() };
			}
			throw new UiWindowsError(
