...
function Get-AppWindows([string]$proc) {
  ...
  foreach ($w in $root.FindAll([System.Windows.Automation.TreeScope]::Descendants, $cWin)) {
    try { if ($ids.ContainsKey([int]$w.Current.ProcessId)) { $out += $w } } catch { }
  }
  return $out
}
function Find-Descendants($el, [int]$max = 4000) {
  ...
  while ($stack.Count -gt 0 -and $seen -lt $max) {
    $n = $stack.Pop(); $seen++
    $n
    # A live UI keeps rebuilding itself under the walk — switching a side panel disposes a whole
    # subtree — and ElementNotAvailableException against $ErrorActionPreference='Stop' aborts the
    # whole script. The caller then sees "powershell exited 1" with the real reason buried in
    # CLIXML progress noise. A vanished branch is not an error: it is a branch that is gone.
    try {
...
/**
 * `env` sets variables on the launched process only — the PowerShell session that starts it is
 * short-lived, so nothing else in the run sees them. Needed for an app that has to be told
 * something before its first line runs; FocuSee's .NET globalization mode is the case here.
 */
export async function launchApp(
	exePath,
	processName,
	{ args = [], timeoutMs = 90_000, env = {} } = {},
) {
	assertWin();
	...
	const setEnv = Object.entries(env)
		.map(([k, v]) => `$env:${k} = ${q(v)}\n`)
		.join("");
	ps(`${setEnv}Start-Process -FilePath ${q(exePath)} ${argList} | Out-Null`);
	const t0 = Date.now();
...
export function clickControl(
	processName,
	needle,
	{ exact = false, controlType = null, byId = false } = {},
) {
	assertWin();
	const typeGuard = controlType
		? `if ($c.ControlType.ProgrammaticName -notmatch ${q(controlType)}) { continue }`
		: "";
	// `byId` exists because a Name can collide with another control's AutomationId, and the
	// walk hands back whichever comes first in the tree. FocuSee's editor has both: the crop
	// tool is named "Export" while the export button's AutomationId is `Export`, so the obvious
	// lookup opens the crop dialog instead of exporting.
	const match = byId
		? `($c.AutomationId -eq ${q(needle)})`
		: exact
	...
	const out = ps(
		...
		if ($hit -eq $null) {
			...
			$p = $null
			# A pattern can be published and still refuse: a tab that is momentarily disabled
			# answers Select() with ElementNotEnabledException. That is an answer about the app,
			# not a reason to lose the whole call, so it comes back as ok=$false with the reason.
			$failed = ''
			try {
			...
			if ($failed) {
				@{ ok = $false; reason = $failed } | ConvertTo-Json -Compress
				exit
			}
			# The control this just invoked may already be gone — a tab that swapped its own panel
			# out, a dialog that closed. Reading its name back is a courtesy, not the result.
			$matched = ''
			try { $matched = $hit.Current.Name } catch { }
...
/** Read a control's value, for verifying a field took what was set. */
		...
		foreach ($w in (Get-AppWindows ${q(processName)})) {
			foreach ($e in (Find-Descendants $w)) {
				...
				if ($c.Name -like ${q(`*${needle}*`)} -or $c.AutomationId -like ${q(`*${needle}*`)}) {
					...
					if ($e.TryGetCurrentPattern([System.Windows.Automation.RangeValuePattern]::Pattern, [ref]$p)) {
						$val = [string]$p.Current.Value; break
					}
				}
			}
...
/**
 * Set a control found by name, without keystroke racing.
 *
 * ValuePattern first, then RangeValuePattern: a WPF slider publishes only the latter, so a
 * padding or shadow control that is a slider rather than a text field answered "no" to every
 * attempt and the scenario was quietly not applied. Both are tried because the caller does not
 * know, and should not have to, which kind of control the app drew.
 */
		...
		foreach ($w in (Get-AppWindows ${q(processName)})) {
			foreach ($e in (Find-Descendants $w)) {
				...
				if ($c.Name -like ${q(`*${needle}*`)} -or $c.AutomationId -like ${q(`*${needle}*`)}) {
					...
					if ($e.TryGetCurrentPattern([System.Windows.Automation.RangeValuePattern]::Pattern, [ref]$p)) {
						# [double] on a string parses invariantly in PowerShell, so this does not
						# depend on the machine's decimal separator.
						$p.SetValue([double]${q(value)}); $done = $true; break
					}
				}
			}
			if ($done) { break }
...
while ([DateTime]::UtcNow -lt $deadline -and $dlg -eq $null) {
  $polls++
  foreach ($w in (Get-AppWindows ${q(processName)})) {
    if ($w.Current.Name -match 'save|open|enregistrer|ouvrir|export|folder|dossier') { $dlg = $w; break }
  }
...
# Every window the app owns, the matched one first — not the matched one alone.
#
# A modern Windows picker is composed: the top-level window named "Ouvrir" held 110 elements
# and none of the buttons, while "Ouvrir" and "Annuler" lived in a sibling window that
# Get-AppWindows also returns. Walking only the name-matched window found the file-name field
# but never a commit button, fell back to Enter, and left a second picker open behind it.
# The win that mattered was doing this in one session rather than four; the narrowing was not
# part of it.
$scan = @($dlg) + @((Get-AppWindows ${q(processName)}) | Where-Object { $_ -ne $dlg })
$edit = $null; $editByName = $null; $btn = $null; $btnName = ''; $buttons = @()
foreach ($e in ($scan | ForEach-Object { Find-Descendants $_ })) {
  $c = $e.Current
  $t = $c.ControlType.ProgrammaticName
  if ($t -match 'Button' -and $c.Name) { $buttons += $c.Name }
  # 1148 is the file-name box of a file dialog, 1152 the folder-name box of a folder picker.
  # Both are language-independent, which the labels are not — and the name fallback below had
  # to drop its bare 'Nom' because a details-view column header is an Edit named exactly that.
  if ($edit -eq $null -and $t -match 'Edit' -and
      ($c.AutomationId -eq '1148' -or $c.AutomationId -eq '1152')) { $edit = $e }
  if ($editByName -eq $null -and $t -match 'Edit' -and
      $c.Name -match 'File name|Nom du fichier') { $editByName = $e }
  # Localised labels first: on this machine "Save" never matches and cost a whole extra walk.
  # AutomationId 1 is IDOK, which is what a folder picker's "Select Folder" answers to under
  # every language and every label the shell invents for it.
  if ($btn -eq $null -and $t -match 'Button' -and
      ($c.Name -match '^&?(Enregistrer|Save|Exporter|Export|Ouvrir|Open|OK)$' -or $c.AutomationId -eq '1')) {
    $btn = $e; $btnName = $c.Name
  }
  if ($edit -ne $null -and $btn -ne $null) { break }
}
if ($edit -eq $null) { $edit = $editByName }
if ($edit -eq $null) {
