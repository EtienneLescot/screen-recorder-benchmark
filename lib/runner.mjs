 ...
 *
 * `minDiskGiB` was 20, which is roughly nine times what a run needs: three tools at three
 * scoring repetitions plus a warm-up, with a paired floor beside every one of them, measured
 * 2.2 GB of output. So it fired on every run on any machine that was not nearly empty, and
 * never once described a real problem. A precondition that is always failing is one nobody
 * reads, which costs more than the check is worth — the interesting warnings in this list are
 * the ones about power and competing load, and they were being printed alongside a permanent
 * one. Six leaves room for a longer run and still catches a genuinely full disk.
 */
export function preconditionCheck({ requireAC = true, minDiskGiB = 6 } = {}) {
...
export async function runOnce(driver, ctx) {
	...
	// A second, non-authoritative completion signal for drivers whose UI explicitly reports
	// success. The filesystem remains the stopwatch stop, but the recorded skew makes systematic
	// early/late signals visible during driver review.
	ctx.observeComplete = (ts = now()) => {
		if (appCompletedAt === null) appCompletedAt = ts;
	};
	/**
	 * A place for a driver to record what it saw the app do, per run.
	 *
	 * Drivers were writing this onto ctx.state, which is shared across a leg and read by
	 * nothing — so the encoder path Recordly prints while it renders, the one piece of evidence
	 * that a "with CUDA" row is not the "without" row again, was collected and dropped on every
	 * run. Anything observed here lands on the record and in results.json.
	 */
	const observed = {};
	...
	if (Object.keys(observed).length) record.observed = observed;
	...
	const wait = await waitForStableFile(out, {
		...
		// "A render that started will put something on disk quickly" is true of every tool here
		// but one. Screen Studio renders frames into a temporary directory and muxes at the end,
		// so its output path stays absent for the whole export and then appears complete — under
		// the four-minute default it was recorded as "output never appeared" for exports that had
		// in fact succeeded. A driver whose app behaves that way says so; the default still
		// protects everything else from waiting out a 45-minute timeout on a missed click.
		appearTimeoutMs: ctx.appearTimeoutMs,
		onTick: ctx.onTick,
	});
	...
	record.completionSignal = driverCompletedAt != null ? "driver" : "file-mtime";
	if (appCompletedAt != null && wait.completedAt != null) {
		record.appFileCompletionSkewMs = Math.round(appCompletedAt - wait.completedAt);
	}
	...
	record.outputMeanVolumeDb = v.meanVolumeDb ?? null;
	...
	if (v.valid) {
		try {
			record.visual = inspectExport(out, ctx.scenario, {
				...
				// A bundle's own camera clip wins over the generated one, exactly as the drivers
				// choose it — checking against an asset the export never saw proves nothing.
...
export async function runApp(
	...
	baseCtx,
	{
		...
		cooldownSec = 45,
		log = () => undefined,
		// Fired immediately before each repetition, and whatever it returns is attached to that
		// repetition's record. The floor uses it to pair a denominator with each export instead of
		// measuring one per leg: within a leg the machine keeps moving, and on a thermally
		// unstable machine the last repetition is not divided by the same machine as the first.
		beforeEach = null,
	} = {},
) {
	...
	for (let i = 0; i < total; i++) {
		...
		const paired = beforeEach ? await beforeEach({ index: i, warmup: isWarmup }) : null;
		const ctx = { ...baseCtx, state, run: { index: i }, commit: () => undefined };
		const rec = await runOnce(driver, ctx);
		if (paired) rec.pairedFloor = paired;
		rec.warmup = isWarmup;
