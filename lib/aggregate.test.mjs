...
/** One machine, one or more tools, each at a known multiple of that machine's floor. */
function submission(costs, { at = "2026-01-01T00:00:00.000Z" } = {}) {
	return {
		submittedAt: at,
		conditions: { driftRatio: 1 },
		scenario: { step: "S4" },
		// Only public-bundle footage is counted, so the fixture has to carry a source like a real
		// submission does.
...
describe("aggregate", () => {
	...
	it("counts a submission carrying a single tool", () => {
		// The ratio graph could do nothing with one measurement: no second tool, no edge, nothing
		// touched. The runner enforced that upstream by skipping the floor entirely unless two
		// apps were listed, which left a one-app run with no denominator at all. Both gates are
		// gone — the floor is measured beside every leg — so a person who owns a licence for one
		// of these products can measure it.
		const r = aggregate([submission({ solo: 2.5 })], { step: "S4" });
		expect(r.skipped).toEqual([]);
		expect(r.tools).toHaveLength(1);
		expect(r.tools[0].relativeCost).toBe(2.5);
	});

	it("ranks tools the machines never measured together, at what each of them measured", () => {
		// a vs b on one machine, b vs c on another, and no machine sees a and c. They are still
		// comparable, because both machines divided by their own ffmpeg — that is what the floor
		// is for, and a roster that fractures by platform would otherwise tell us nothing.
		//
		// Every figure is what was measured. The ratio graph answered 4 here instead of 8: it
		// read b disagreeing with itself (2.0 against 4.0) as proof that the second machine ran
		// everything twice as dear, and divided c by that. The corpus says there is no such
		// single factor — between an M1 and a Ryzen laptop OpenScreen moves 1.8× while Recordly
		// moves 5.2× on the same pair — and inventing one moved published figures outside every
		// run behind them.
		...
		expect(cost.c).toBe(8);
		// b was measured on both and disagrees with itself. The row says 3, and the range says why.
		expect(cost.b).toBe(3);
		expect(result.tools.find((t) => t.tool === "b").observedCostRange).toEqual([2, 4]);
	});
...
describe("a version is a competitor, not a label", () => {
	...
	it("shows the newest build of each tool and leaves the rest to the builds page", () => {
		const at = (platform, cost) => ({ platform, machine: platform, gpu: platform, runs: 1, cost });
		const rows = newestPerPlatform([
			{ node: "openscreen@1.10.0", tool: "openscreen", build: "1.10.0", setups: [at("win32", 4)] },
			{
				node: "openscreen@1.11.0-rc",
				tool: "openscreen",
				build: "1.11.0-rc",
				setups: [at("win32", 1)],
			},
			{ node: "cap@0.5.9", tool: "cap", build: "0.5.9", setups: [at("win32", 2)] },
		]);
		expect(rows.map((t) => t.build).sort()).toEqual(["0.5.9", "1.11.0-rc"]);
		// One build per row is the ordinary case, and the row still names exactly one.
		expect(rows.every((t) => t.builds.length === 1)).toBe(true);
	});

	/**
	 * The failure this exists for, and it shipped.
	 *
	 * FocuSee 2.4.1 was measured on an M1 and 2.3.5 on two Windows machines. Taking the newest
	 * build *anywhere* made the headline row a macOS-only figure — 0.93x, one run, one machine,
	 * ranked first — while both Windows machines, one of which ran 10.7x, fell out of the chart
	 * entirely because they belonged to an older build. Any vendor shipping at different
	 * cadences per platform reproduces this.
	 */
	it("takes the newest build measured on each platform, not the newest anywhere", () => {
		const at = (platform, machine, cost) => ({ platform, machine, gpu: machine, runs: 1, cost });
		const [row] = newestPerPlatform([
			{
				node: "focusee@2.4.1",
				tool: "focusee",
				build: "2.4.1",
				setups: [at("darwin", "M1", 0.93)],
				observedCostRange: [0.93, 0.93],
			},
			{
				node: "focusee@2.3.5",
				tool: "focusee",
				build: "2.3.5",
				setups: [at("win32", "5800X", 1.15), at("win32", "7520U", 10.73)],
				observedCostRange: [1.15, 10.73],
			},
		]);
		// Every setup that ran its own platform's current release, counted once each.
		expect(row.setups).toHaveLength(3);
		expect(row.relativeCost).toBe(+((0.93 + 1.15 + 10.73) / 3).toFixed(3));
		expect(row.machines.sort()).toEqual(["5800X", "7520U", "M1"]);
		// Both builds are named, each against the platforms it answers for, because one version
		// number over this row would be true of only some of the runs under it.
		expect(row.builds).toEqual([
			{ build: "2.4.1", prerelease: undefined, platforms: ["darwin"] },
			{ build: "2.3.5", prerelease: undefined, platforms: ["win32"] },
		]);
		// The range covers every run in the row, not just the newest build's.
		expect(row.observedCostRange).toEqual([0.93, 10.73]);
	});

	it("is plain newest-build selection inside a scope that is one platform", () => {
		const at = (cost) => ({ platform: "win32", machine: "5800X", gpu: "rtx", runs: 1, cost });
		const rows = newestPerPlatform([
			{ node: "focusee@2.4.1", tool: "focusee", build: "2.4.1", setups: [at(0.93)] },
			{ node: "focusee@2.3.5", tool: "focusee", build: "2.3.5", setups: [at(1.15)] },
		]);
		expect(rows).toHaveLength(1);
		expect(rows[0].build).toBe("2.4.1");
		expect(rows[0].relativeCost).toBe(0.93);
	});
...
describe("consistency separates a repeat from a change of hardware", () => {
	...
	it("reads two runs of one machine as the repeatability figure", () => {
		...
		// Two builds, each measured twice on the one setup. Counted per build now rather than per
		// pair of tools: a build measured twice disagrees with itself, and needs no rival to say so.
		expect(r.consistency.byPlatform.sameGpuRepeats).toBe(2);
	...
	it("does not count two different GPUs as a repeat", () => {
		...
		expect(r.consistency.byPlatform.sameGpuRepeats).toBe(0);
		expect(r.consistency.byPlatform.samePlatformRepeats).toBe(2);
	...
	it("keeps a same-GPU repeat tighter than the same-platform spread around it", () => {
		...
		expect(bp.sameGpuRepeats).toBe(2);
 ...
 *
 * Two failures shipped together on the published page, and dropping the ratio graph is what
 * fixed both. A scope cut to one machine came apart into two disconnected groups and still
 * numbered them 01 to 04 in a single list, so a tool at 3.04× sat above one at 1.15× and read as
 * a sorting bug. And a build measured on one machine showed a page-wide figure of 1.22× while
 * every run of it measured between 1.145× and 1.157×, the row carrying a tag to admit the number
 * was a placement rather than a measurement. Neither can happen against a floor every submission
 * carries; these hold that shut.
 */
describe("the ranking says what its figures are", () => {
	it("puts tools that never met in one list, because both divided by ffmpeg", () => {
		// Two pairs, no tool in common: a and b were measured together, c and d were measured
		// together, and nothing joins the pairs. Under the graph they were two components on two
		// scales, and the chart had to break itself in half to say the halves did not compare.
		const r = aggregate([submission({ a: 4.0, b: 5.0 }), submission({ c: 1.1, d: 1.3 })], {
			step: "S4",
		});
		expect(r.tools.map((t) => t.tool)).toEqual(["c", "d", "a", "b"]);

		const html = renderSite(r, { submissions: [], generatedAt: "2026-01-01", roster: [] });
		expect(html).not.toMatch(/class="rank-split"/);
		expect(html.match(/class="rank-no">01</g)).toHaveLength(1);
	});

	it("never places a build outside its own runs", () => {
		// The case that used to break it: `lonely` was held into the graph only by `bridge`, and
		// `bridge` is dear on a second machine `lonely` never ran on. The solve kept their ratio
		// and lifted the level, landing `lonely` above every run of itself.
		...
		expect(lonely.relativeCost).toBe(1.15);

		// Every row, not just that one: a median cannot leave its own sample.
		for (const t of r.tools) {
			const [lo, hi] = t.observedCostRange;
			expect(t.relativeCost, t.node).toBeGreaterThanOrEqual(lo);
			expect(t.relativeCost, t.node).toBeLessThanOrEqual(hi);
		}
		const html = renderSite(r, { submissions: [], generatedAt: "2026-01-01", roster: [] });
		expect(html).not.toMatch(/its (run|runs)</);
	});

	it("counts a machine once, however many runs it submitted", () => {
		// Otherwise whoever submits most decides the figure. OpenScreen 1.11.0-rc ran four times
		// on one M1 at 1.04× and once on a Ryzen laptop at 1.44×; a flat median over the runs
		// returns the M1 number, which is a fact about the submitter and not about the tool. Each
		// setup is pooled to its own median first, so four runs and one run weigh the same.
		const fast = { platform: "win32", chip: "Ryzen 7 5800X", gpu: "NVIDIA GeForce RTX 4070 Ti" };
		const slow = { platform: "linux", chip: "Ryzen 5 7520U", gpu: "AMD Radeon(TM) Graphics" };
		const r = aggregate(
			[
				onMachine({ a: 1.0, b: 2.0 }, fast),
				onMachine({ a: 1.0, b: 2.0 }, { ...fast, at: "2026-01-02T00:00:00.000Z" }),
				onMachine({ a: 1.0, b: 2.0 }, { ...fast, at: "2026-01-03T00:00:00.000Z" }),
				onMachine({ a: 3.0, b: 6.0 }, { ...slow, at: "2026-01-04T00:00:00.000Z" }),
			],
			{ step: "S4" },
		);
		const a = r.tools.find((t) => t.tool === "a");
		expect(a.setups).toHaveLength(2);
		// Two setups at 1.0 and 3.0, so the midpoint — not the 1.0 that three-to-one would give.
		expect(a.relativeCost).toBe(2);
	});

	it("leaves a figure that sits inside its own runs unmarked", () => {
		// The mark is gone, and so is the shape of page that carried it.
