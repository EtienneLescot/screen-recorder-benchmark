 ...
 * desktops and the Apple part weighted ×1 and the only thermally-limited laptop weighted ×0.6
 * and ×0.25 — drift the sole reason in every case, on a benchmark whose whole claim is that a
 * cost in units of the floor travels between machines.
 */
describe("submissionWeight reads the error on the denominator, not the machine", () => {
	const conditions = (extra) => ({
		submittedAt: "2026-01-01T00:00:00.000Z",
		conditions: { onACPower: true, remoteDesktopActive: false, ...extra },
		measurements: [],
	});

	it("ignores run-wide drift when the per-leg floors held", () => {
		// The run that prompted this: 19.9% across the run, 4.26% within the widest leg, and a
		// software floor that moved 0.8% over the same 90 minutes.
		const w = submissionWeight(conditions({ driftRatio: 1.1987, floorSpreadPercent: 4.26 }));
		expect(w.weight).toBe(1);
		expect(w.reasons).toEqual([]);
	});

	it("still penalises a denominator that moved under the tool it was dividing", () => {
		expect(submissionWeight(conditions({ floorSpreadPercent: 9 })).weight).toBe(0.6);
		expect(submissionWeight(conditions({ floorSpreadPercent: 22 })).weight).toBe(0.25);
	});

	/** The metric needs paired floors, which submissions published before it never carried. */
	it("falls back to drift for a submission that cannot report the new metric", () => {
		const old = submissionWeight(conditions({ driftRatio: 1.097 }));
		expect(old.weight).toBe(0.25);
		expect(old.reasons[0]).toMatch(/drift/);
	});

	/** Bias, not noise: these shift the estimate rather than widen it, so they still apply. */
	it("keeps the penalties for conditions nothing corrects", () => {
		expect(submissionWeight(conditions({ remoteDesktopActive: true })).weight).toBe(0.3);
		expect(submissionWeight(conditions({ onACPower: false })).weight).toBe(0.5);
	});
});
