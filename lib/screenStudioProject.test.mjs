...
/**
 * The output frame is the axis this app derives rather than obeys. Left on its default the
 * composition decides it — a 1920×1080 source at padding 10 composes 2220×1380, and "1080p"
 * exports 1736×1080: a conforming file, at 60 fps, with every effect present, and the wrong
 * frame. It was measured coming out that way before this was pinned.
 */
describe("the output frame is pinned to the scenario's aspect", () => {
	it("reduces the target to the app's own ratio pair", () => {
		expect(aspectRatioFor({ width: 1920, height: 1080 })).toEqual({ x: 16, y: 9 });
		expect(aspectRatioFor({ width: 1280, height: 720 })).toEqual({ x: 16, y: 9 });
		expect(aspectRatioFor({ width: 1080, height: 1080 })).toEqual({ x: 1, y: 1 });
	});

	it("writes it into the project rather than leaving the app on Auto", () => {
		expect(config().config.defaultOutputAspectRatio).toEqual({ x: 16, y: 9 });
	});
});
