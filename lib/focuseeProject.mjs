...
/**
 * How the recording's sound is staged for the app. Stereo, not a copy of the source's
 * channels: FocuSee 2.3.5 reads a mono sidecar as stereo pairs and exports half the
 * duration (60s in, 30s out, same tempo and pitch — the back half is silence). Duplicating
 * the channel up front carries the identical content and exports whole.
 */
export const systemAudioArgs = (sourcePath, outPath) => [
	"-y",
	"-i",
	sourcePath,
	"-vn",
...
export function writeFocuseeProject({
	...
	if (ffmpeg) {
		try {
			execFileSync(ffmpeg, systemAudioArgs(sourcePath, join(rec, "systemaudio-0.m4a")), {
				stdio: "ignore",
			});
			recorders.push({
