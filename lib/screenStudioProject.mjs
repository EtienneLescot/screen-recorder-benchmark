...
function buildRecording({ recordingPath, sourcePath, webcamPath, cursorSamples }) {
	...
	if (webcamPath && existsSync(webcamPath)) {
		const cam = placeVideo(webcamPath, join(recordingPath, RECORDING_FILES.webcam));
		webcam = RECORDING_FILES.webcam;
		webcamSize = { width: cam.probe.video.width, height: cam.probe.video.height };
	}
				...
				? [
						{
							configuration: {},
							id: "channel-2-webcam",
							// `videoSize` is not decoration and its absence is not survivable. The
							// editor loads a camera without it perfectly happily — the Camera panel
							// shows the size and roundness, and every read-back agrees — and then the
							// *compositor* reads `session.webcam.videoSize.width` in three places
							// (the inset's aspect ratio, its fit, and the roundness in pixels) and
							// dies with "Cannot read properties of undefined (reading 'width')". The
							// export silently stops seconds in: no file, no dialog, no toast, and an
							// app left holding a half-started export that refuses every later one
							// with "Cannot start export, there is already an export in progress".
							//
							// Nothing before the export can catch this, which is exactly why it
							// survived a machine that could not export at all.
							sessions: [session({ outputFilename: webcam, videoSize: webcamSize })],
							type: "webcam",
						},
					]
				: []),
 ...
 */
/** 1920×1080 → {x:16, y:9}. The app accepts `{x,y}` or `[x,y]`; its own documents use `{x,y}`. */
export function aspectRatioFor({ width, height }) {
...
export function buildConfig(scenario, { paddingControl, wantsCursor, wantsCamera, durationMs }) {
	...
	const config = {
		/**
		 * The one axis whose absence does not look like an absence.
		 *
		 * Left null — the app's own default, "Auto" — the output frame is *derived from the
		 * composition*: `screenWithInset + 2 × ceil(avg(screen) × padding/100)`. A 1920×1080
		 * source at padding 10 therefore composes 2220×1380, and "1080p" exports that aspect:
		 * 1736×1080. A conforming H.264 file at 60 fps with the right duration and every effect
		 * present — and the wrong frame, failing the one thing this benchmark forces to be
		 * identical.
		 *
		 * Pinning it makes the app expand the background to the scenario's aspect instead
		 * (`expandToMatchAspectRatio`), which is what every other tool here does with its frame.
		 * Measured before it was pinned: 1736×1080 out of a run that claimed `targetResolution`.
		 */
		defaultOutputAspectRatio: aspectRatioFor(scenario.output),
