...
${
	...
	c?.measurable
		? (
				() => {
					// The number and where to look for your own hardware — nothing about what the
					// spread means. Earlier drafts of this note explained it away (encoders, GPUs,
					// drivers, "no obligation to lead on another stack"), which is the author of an
					// entrant telling the reader how to read a result that did not go one way.
					// The measurement is publishable; the reading of it is the reader's.
					const w = c.worst;
					const example = w
    ...
    <div class="aside after">
      <p class="lead"><strong>The same pair does not give the same ratio on every machine.</strong>
      Where a pair was measured on more than one, its ratio moves by ${c.medianResidualPercent}% at
      the median.</p>
      <p class="fine">${example}The chart above combines every machine at once; the buttons above it
      rank one platform or one GPU vendor at a time. Only pairs measured on more than one machine
      are counted in this figure.</p>
    </div>`;
				}
			)()
		: `
    <div class="aside after">
      ...
      machine.</p>
      <p class="fine">Nothing here has been measured twice, so no cross-check has run yet. A
      second submission covering an overlapping pair makes one possible.</p>
    </div>`
}
${
	superseded > 0
    ...
    <div class="aside after">
      ...
      still in the solve.</p>
      <p class="fine">Builds are ranked separately rather than averaged together.
      <a href="builds.html">Every build measured</a> lists them all, and lets you pick one build
      per tool.</p>
    </div>`
		: ""
}${
    ...
    <div class="aside after">
      ...
      <p class="fine">${result.buildSpread.map(say).join("; ")}. The runs behind each are in the
      table below.</p>
    </div>`;
				}
			)()
    ...
    <div class="aside after">
      ...
      missing version is a defect in the submitting harness, and those runs have to be
      re-measured.</p>
    </div>`
		: ""
}
