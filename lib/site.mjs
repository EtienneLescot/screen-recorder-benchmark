 ...
 * how many machines each figure rests on, how far a build's cost moves between them, and who
 * submitted what.
 *
 ...
 *
 * Where a vendor ships at different cadences per platform the row rests on more than one build,
 * and then every one of them is named against the platforms it answers for: "2.4.1 (macOS) ·
 * 2.3.5 (Windows)". One version number over a row that is half macOS 2.4.1 and half Windows
 * 2.3.5 would be true of only some of the runs under it. Where the platforms agree — the usual
 * case — there is one build and the chip reads exactly as it always did.
 */
const rcTag =
	' <span class="tag" title="a prerelease — published by the vendor, but not the release build">rc</span>';
const buildChip = (t) => {
	const parts = t.builds ?? (t.build ? [{ build: t.build, prerelease: t.prerelease }] : []);
	if (!parts.length) return "";
	if (parts.length === 1)
		return ` <span class="dim">${esc(parts[0].build)}</span>${parts[0].prerelease ? rcTag : ""}`;
	const named = parts
		.map((p) => `${esc(p.build)} (${p.platforms.map(platformName).map(esc).join(", ")})`)
		.join(" · ");
	return (
		` <span class="dim" title="this tool is not on the same release everywhere it was measured — each platform contributes the newest build measured on it">${named}</span>` +
		(parts.some((p) => p.prerelease) ? rcTag : "")
	);
};
...
function rankChart(tools, { flagThin = true } = {}) {
	...
	// One list, always. Under the ratio graph two tools never measured together fell into
	// separate components, each solved on its own scale, and 01 in one was not faster than 02 in
	// the other — so the chart had to break itself in half and say why. Against the floor there
	// is one scale for everything, because every cost was divided by an ffmpeg run on its own
	// machine. Nothing can be disconnected from a denominator every submission carries.
	const row = (t, i) => {
		...
		const tag = thin
			...
			: "";
		// No "outside its own runs" tag any more, because a figure can no longer land there: the
		// number is the median of this build's own measurements. It could before. A build held
		// into the ratio graph by a single neighbour inherited that neighbour's level, fitted
		// across machines it never ran on, and Screen Studio ran once at 4.681× while the page
		// printed 5.70×. The tag disclosed that; removing the graph removed the need for it.
		//
		// What one number still cannot say is how far a build moves between machines. The
		// observed range is in the run table and the per-platform scopes are the honest read.
		//
        ...
        <div class="rank-head">
          ...
          <h3>${esc(displayTool(t.tool))}</h3>${buildChip(t)}${tag}
 ...
 *
 * The pool is what the aggregate actually counted, not everything in `submissions/`. It
...
function scopeNote(scope) {
	...
	const tools = newestPerPlatform(scope.res.tools);
	if (!tools.length) return "";
...
${stripRows}
	...
	const w = c?.worst;
	const worstBuild = w
		? ` — widest: ${esc(displayTool(w.tool))}, ${w.ends[0].cost}× on ${esc(machineLabel(w.ends[0].machine))} and ${w.ends[1].cost}× on ${esc(machineLabel(w.ends[1].machine))}`
		: "";
	...
	// The headline ranks one row per tool: the newest build measured on each platform.
	//
	// Every build gets its own figure — a release that got three times faster is the whole
	...
	// question nobody asked; a reader wants to know what to install today. So every build is
	// published and the chart shows the current one. The rest are two clicks away and one
	// section down, never deleted.
	//
	// "Current" is per platform rather than global, because a vendor shipping at different
	// cadences per platform otherwise empties a row of every run made on the platforms it is
	// behind on — see . Where a row rests on more than one build the chip
	// names each against its platforms.
	const ranked = (res) => newestPerPlatform(res.tools);
	// Builds genuinely absent from the chart, not builds minus rows. A row can name two builds
	// now — one per platform where the vendor ships at different cadences — and counting rows
	// would call FocuSee 2.3.5 "not shown" on a page that is showing it.
	const shown = new Set(ranked(result).flatMap((t) => t.builds.map((b) => `${t.tool}@${b.build}`)));
	const superseded = result.tools.filter((t) => !shown.has(t.node)).length;
...
${note ? `      <p class="scope-note">${esc(note)}</p>\n` : ""}      ${s.res.tools.length ? rankChart(ranked(s.res), { flagThin: machines.size > 1 }) : emptyScope(s, roster)}
    ...
    <div class="tri">
      ...
      <p><strong>No tool is the reference.</strong> Every export is divided by an ffmpeg
      transcode on its own machine, so no submission has to contain any particular tool.</p>
    </div>
${
	// Gated on `measurable`, not on the numbers being present. A build measured once agrees
	// with itself perfectly, and printing that 0% as agreement would claim a cross-check that
	// never ran.
	[
		c?.measurable
			? `Cross-check: where a build was measured on more than one machine, its cost moves by ${c.medianSpreadPercent}% at the median${worstBuild}.`
			: "Cross-check: none — no build here has been measured twice.",
		superseded > 0
			? `${plural(superseded, "older build")} not shown: each row is the newest build measured, and the ones it replaced keep their own figures. <a href="builds.html">Every build measured →</a>`
			: "",
 ...
 *
 * The filter hides rows; it does not recompute. Every build's figure is the median of its own
 * runs against ffmpeg, so two rows are comparable whether or not a third is on screen and
 * ticking a box can never change a number. Under the ratio graph this was a real hazard —
 * re-solving per selection returned a different figure for the same pair depending on what else
 * was showing — and against a fixed denominator it cannot arise.
 */
