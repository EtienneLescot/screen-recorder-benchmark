...
/**
 * A machine, for grouping — the chip *and* the platform it was measured under.
 *
 * The 7520U in the tree is one physical box that submitted under both Windows and Linux, and its
 * two operating systems disagree about which tool wins. Keyed on the chip alone they pool into a
 * single "Ryzen 5 7520U" ranking that is a cross-OS consensus wearing a machine's name, which is
 * the one thing this page exists to avoid. A ratio belongs to a machine under an OS.
 */
const machineKey = (sub) =>
	JSON.stringify([machineLabel(sub?.machine?.chip), sub?.machine?.platform ?? sub?.platform ?? ""]);
...
// Two vocabularies reach this map: Node's platform ids from the submissions, and the roster's
// own column names. Both land on the same three words.
...
/**
 * What each automation rung actually is.
 *
 * The column has always been on the page and has never said what it means, which is the worst of
 * both: a reader who notices that one tool is driven unlike the rest gets the fact without the
 * explanation, and fills the gap themselves. Only the rungs present are described.
 */
const RUNG = {
	cli: "scripted headlessly",
	cdp: "driven through the app's own debugging port",
	uia: "driven through the Windows UI Automation API",
	menu: "driven by System Events through named menu items",
	"menu+coords": "driven by menu items plus pixel clicking, the least reproducible rung",
};
...
/** "macOS and Linux", "macOS, Linux and Windows" — a list a sentence can swallow. */
const listWords = (xs) =>
...
/**
 * What to call one machine-under-an-OS, disambiguated only when it has to be.
 *
 * "Ryzen 5 7520U" is enough until the same chip appears under two platforms, at which point the
 * OS is the whole difference between the two rows and has to be on the label.
 */
function contextName(pool, key) {
	const [chip, platform] = JSON.parse(key);
	const spans =
		new Set(
			pool
				.filter((s) => machineLabel(s.machine?.chip) === chip)
				.map((s) => s.machine?.platform ?? s.platform),
		).size > 1;
	// The label goes in a button and a list, the tail into the middle of a sentence, and
	// "leads on Ryzen 5 7520U on Windows" is not a sentence anybody wrote on purpose.
	return spans
		? {
				label: `${chip} (${platformName(platform)})`,
				tail: `on ${chip} under ${platformName(platform)}`,
			}
		: { label: chip, tail: `on ${chip}` };
}

/**
 * The finer cut a scope's own claim has to survive.
 *
 * A consensus across platforms is checked against each platform; a scope that is already one
 * platform is checked against each machine on it. Anything with a single part cannot contradict
 * itself and returns nothing, which is how a single-machine scope stays a plain sentence.
 */
function partitionOf(subs, step, pool) {
	const platformOf = (s) => s.machine?.platform ?? s.platform ?? "";
	const platforms = [...new Set(subs.map(platformOf))];
	const byPlatform = platforms.length > 1;
	const kind = byPlatform ? "platform" : "machine";
	const keys = byPlatform
		? platforms
				.sort(cmp)
				.map((pf) => ({ label: platformName(pf), match: (s) => platformOf(s) === pf }))
		: [...new Set(subs.map(machineKey))].sort(cmp).map((k) => ({
				label: contextName(pool, k).label,
				match: (s) => machineKey(s) === k,
			}));
	if (keys.length < 2) return { kind, parts: [] };
	const parts = keys
		.map(({ label, match }) => {
			const tools = aggregate(subs.filter(match), { step }).tools;
			return { label, leader: tools.length > 1 ? tools[0].tool : null };
		})
		.filter((p) => p.leader);
	return { kind, parts };
}
...
function buildScopes(submissions, { result, step }) {
	...
	const platformOf = (s) => s.machine?.platform ?? s.platform ?? "";
	const groups = [
		{ name: "", items: [{ id: "all", label: "All", tail: "overall" }] },
		{
			name: "Platform",
		...
		{
			name: "Machine",
			items: [...new Set(pool.map(machineKey))].sort(cmp).map((k, i) => ({
				id: `m-${i}`,
				...contextName(pool, k),
				filter: (s) => machineKey(s) === k,
			})),
		},
	];
	...
	for (const group of groups) {
		...
		for (const item of group.items) {
			...
			const res = item.id === "all" ? result : aggregate(subs, { step });
			scopes.push({
				...item,
				subs,
				res,
				// What this scope's own claim could be contradicted by, one level down. A consensus
				// over several platforms is checked against each platform; a single platform against
				// each machine on it. The leader of a scope that loses inside its own partition has
				// to say so in the same sentence, or the reader finds out by clicking.
				partition: partitionOf(subs, step, pool),
				// Resolved here, where the whole pool is in hand: whether a chip needs its OS on
				// the label is a fact about the tree, not about the subset being described.
				machineNames: [...new Set(subs.map(machineKey))]
					.sort(cmp)
					.map((k) => contextName(pool, k).label),
...
function scopeNote(scope) {
	...
	const chips = [...new Set(scope.subs.map((s) => machineLabel(s.machine?.chip)))].sort(cmp);
	const contexts = [...new Set(scope.subs.map(machineKey))].sort(cmp);
	const platforms = [
		...new Set(scope.subs.map((s) => s.machine?.platform ?? s.platform).filter(Boolean)),
	];
	if (scope.id === "all") {
		return `All ${plural(legs, "run")} — ${plural(tools.length, "tool")}, ${plural(chips.length, "machine")}, ${plural(platforms.length, "platform")}. Combined as a graph of ratios, so no tool is the denominator.`;
	}
	const named = scope.machineNames.join(" · ");
	// One box that submitted under two operating systems is not two machines, and a scope that
	// pools them is not twice as well covered as one that does not. Counting contexts and saying
	// so is the difference between coverage and the appearance of it.
	const caveat =
		chips.length === 1 && contexts.length > 1
			? " One machine under two operating systems, not two machines."
			: contexts.length === 1
				? " One machine is a data point, not a verdict."
				: "";
	return `${named} — ${plural(legs, "run")}, ${plural(tools.length, "tool")}.${caveat}`;
}

/** A scope nothing has been measured in, explained from the roster rather than left blank. */
function emptyScope(scope, roster) {
	const where = scope.tail;
		...
		(sub.measurements ?? [])
			...
			.map((m) => {
				...
				return {
					...
					// Folded, because the same box reports itself differently depending on who is
					// asking: the 7520U arrives padded to a fixed width from Windows and unpadded
					// from Linux. Counted raw it is two machines, and the filter offers the same
					// name twice selecting different halves of one box.
					machineName: machineLabel(sub.machine?.chip),
					gpu: sub.machine?.gpu ?? "",
...
${stripRows}
	...
	const machines = [...new Set(runRows.map((r) => r.machineName))].sort(cmp);
	const platforms = [...new Set(runRows.map((r) => r.platform).filter(Boolean))].sort(cmp);
	const runTable = sortedRuns
		.map(
			(
				r,
			) => `          <tr data-machine="${esc(r.machineName)}" data-platform="${esc(r.platform)}" data-tool="${esc(r.tool)}">
            <td class="nw"><strong>${esc(displayTool(r.tool))}</strong>${r.version ? ` <span class="dim">${esc(r.version)}</span>` : ""}</td>
            <td class="nw" title="${esc(r.machine.replace(/\s+/g, " ").trim())}${r.gpu ? ` · ${esc(r.gpu)}` : ""}">${esc(r.machineName)}</td>
	...
	/**
	 * The Driven column, explained on the page instead of in a file nobody reading it will open.
	 *
	 * A reader who sees that the last-placed tool is also the only one driven through a UI, on a
	 * benchmark published by a competitor of it, is owed the arithmetic rather than an assurance.
	 * The numbers below come from the measurement recorded in `lib/report.mjs`: they are a one-off
	 * characterisation of the harness, not a figure that moves with each submission, so they are
	 * written here as prose. Everything about *which* tool is the odd one out is derived, because
	 * that does move — a second `cdp` entrant would make the sentence false.
	 */
	const drivenNote = (() => {
		const rungs = new Map();
		for (const r of runRows) {
			if (!r.automation) continue;
			if (!rungs.has(r.automation)) rungs.set(r.automation, new Set());
			rungs.get(r.automation).add(displayTool(r.tool));
		}
		if (!rungs.size) return "";
		const glossary = [...rungs.keys()]
			.sort(cmp)
			.filter((k) => RUNG[k])
			.map((k) => `<code>${esc(k)}</code> is ${RUNG[k]}`)
			.join("; ");
		// The question only has a sharp form when exactly one entrant is driven unlike the rest.
		const odd = [...rungs].filter(([, tools]) => tools.size === 1 && rungs.size > 1);
		const minority = odd.find(([, tools]) => {
			const only = [...tools][0];
			return [...rungs].every(([, other]) => other === tools || !other.has(only));
		});
		if (!minority) return glossary ? `<p><strong>How it was driven.</strong> ${glossary}.</p>` : "";
		const tool = [...minority[1]][0];
		const ranked = result.tools.map((t) => displayTool(t.tool));
		const last = ranked.length > 1 && ranked[ranked.length - 1] === tool;
		return `<p><strong>How it was driven.</strong> ${glossary}. <code>${esc(tool)}</code> is the
      only entrant driven that way${last ? ", and it also ranks last" : ""}, so what the rung costs
      it is measured rather than asserted. Everything the automation does to set up an export —
      launching, loading the project, pinning the export settings — finishes before the clock
      starts. What remains inside the window is a progress poll: one text read every 2 s, about
      14 ms across a 28.7 s render, or 0.05% of the row. The CPU-seconds beside it are the app's
      own renderer, not the harness.</p>`;
	})();
	...
	/**
	 * The one sentence a reader is most likely to quote, and the one most able to mislead.
	 *
	 * It used to read "<leader> exports fastest across every run measured so far", which was not
	 * true of this tree and would not have been true of most trees: the consensus leader here loses
	 * to another tool on two of the three platforms, and the single global figure is carried by the
	 * third. Anybody clicking the next button along saw the contradiction immediately, and on a
	 * benchmark whose author ships an entrant that turns the whole page into an argument against
	 * itself.
	 *
	 * So the claim and its defeats share a sentence. The scope's own partition — platforms under a
	 * cross-platform scope, machines under a single-platform one — is checked for a different
	 * leader, and every one found is named here rather than left to be discovered. Losing in
	 * public is the strongest thing an author-run benchmark can do; it only works if the loss is
	 * in the headline instead of one click away.
	 */
	const leadLine = (tools, tail, partition = { kind: "platform", parts: [] }) => {
		if (!tools.length) return `Nothing has been measured ${esc(tail)}.`;
		const leader = tools[0].tool;
		const name = `<strong>${esc(displayTool(leader))}</strong>`;
		if (tools.length === 1) return `Only ${name} has been measured ${esc(tail)}.`;

		const lead = `${name} leads ${esc(tail)} at ${tools[0].relativeCost.toFixed(2)}×`;
		// Grouped by who does the beating: two platforms lost to the same tool read as one fact,
		// not two.
		const beaten = new Map();
		for (const part of partition.parts.filter((p) => p.leader !== leader)) {
			if (!beaten.has(part.leader)) beaten.set(part.leader, []);
			beaten.get(part.leader).push(part.label);
		}
		if (beaten.size) {
			const clauses = [...beaten].map(
				([tool, where]) =>
					`<strong>${esc(displayTool(tool))}</strong> is faster on ${esc(listWords(where))}`,
			);
			return `${lead}, but ${listWords(clauses)}.`;
		}
		// No dissent is only worth asserting where there was something to dissent — one machine
		// agreeing with itself is not a second opinion.
		const n = partition.parts.length;
		return n > 1
			? `${lead}, and on ${n === 2 ? "both" : `each of the ${n}`} ${partition.kind}s measured.`
			: `${lead}.`;
	};
...
${
	scopes.length
		? `${scopes
				.map(
					(s, i) =>
						`    <p class="lead-line" data-scope="${s.id}"${i ? " hidden" : ""}>${leadLine(s.res.tools, s.tail, s.partition)}</p>`,
				)
				.join("\n")}
...
${note ? `      <p class="scope-note">${esc(note)}</p>\n` : ""}      ${s.res.tools.length ? rankChart(s.res.tools, { flagThin: machines.size > 1 }) : emptyScope(s, roster)}
	...
	.join("\n")}`
		: result.tools.length
			? `    <p class="lead-line">${leadLine(result.tools, "overall")}</p>
...
${runTable || '          <tr><td colspan="13" class="dim">No runs yet.</td></tr>'}
    ...
    <div class="tri tight">
      ...
      don't read their raw seconds against each other — and <strong>Cost does not repair it</strong>:
      the floor runs on the fixed-function encoder block, which barely throttles and is largely
      indifferent to CPU contention, while the compositing under test runs on cores and shaders,
      which are not. Measured on one machine, background load doubled between two runs while the
      floor moved 1.1% and the export it was dividing moved 19%.</p>
    </div>
  </section>
