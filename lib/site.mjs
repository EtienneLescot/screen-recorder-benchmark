...
function rankChart(tools, { flagThin = true } = {}) {
	...
	const pct = (v) => `${(((v - 1) / (axis - 1)) * 100).toFixed(1)}%`;
...
function buildScopes(submissions, { result, step }) {
	...
	const groups = [
		...
		{
			name: "GPU",
			items: [...new Set(submissions.map((s) => gpuVendor(s.machine?.gpu)).filter(Boolean))]
				.sort(cmp)
				.map((v) => ({
					id: `g-${v.toLowerCase()}`,
					label: v,
		...
		{
			name: "Machine",
			items: [...new Set(submissions.map((s) => machineLabel(s.machine?.chip)))]
				.sort(cmp)
				.map((m, i) => ({
					id: `m-${i}`,
					label: m,
	...
	for (const group of groups) {
		...
		for (const item of group.items) {
			const subs = item.filter ? submissions.filter(item.filter) : submissions;
			const key = subs.map((s) => submissions.indexOf(s)).join(",");
...
(() => {
  ...
  try {
    const saved = localStorage.getItem("srb-theme");
