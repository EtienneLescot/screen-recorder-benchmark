...
import { aggregate, compareBuilds, newestBuilds, normaliseVersion } from "./aggregate.mjs";
 ...
 *
 * A node id is `openscreen-cli@1.11.0-rc`, which is precise and unreadable. The name and the
...
function buildScopes(submissions, { result, step }) {
	...
	const groups = [
		...
		{
			name: "Machine",
			// Keyed on the chip *and* the platform. The Ryzen 5 7520U in this tree dual-boots, and
			// keyed on the chip alone its Windows and Linux runs pooled into one scope labelled with
			// a machine's name — a cross-OS consensus wearing the label of a single box, which is the
			// one thing the scopes exist to prevent. A chip seen under one platform keeps its plain
			// name; only a chip that appears under more than one says which.
			items: (() => {
				const platformsPerChip = new Map();
				for (const s of pool) {
					const chip = machineLabel(s.machine?.chip);
					if (!platformsPerChip.has(chip)) platformsPerChip.set(chip, new Set());
					platformsPerChip.get(chip).add(s.machine?.platform ?? s.platform ?? "");
				}
				// NUL-joined: a chip label is full of spaces, so any printable separator would
				// split "Ryzen 5 7520U" apart before it reached the platform.
				const key = (s) =>
					`${machineLabel(s.machine?.chip)} ${s.machine?.platform ?? s.platform ?? ""}`;
				return [...new Set(pool.map(key))].sort(cmp).map((k, i) => {
					const [chip, pf] = k.split(" ");
					const label =
						platformsPerChip.get(chip)?.size > 1 ? `${chip} (${platformName(pf)})` : chip;
					return {
						id: `m-${i}`,
						label,
						tail: `on ${label}.`,
...
function scopeNote(scope) {
	...
	if (scope.id === "all") {
		return `All ${plural(legs, "run")} — ${plural(tools.length, "tool")}${over}, ${plural(machines.length, "machine")}, ${plural(platforms.length, "platform")}. Combined as a graph of ratios, so no tool is the denominator.`;
	}
...
${
	superseded > 0
		? `
    <div class="aside after">
      <p class="lead"><strong>${plural(superseded, "older build")} ${superseded === 1 ? "is" : "are"} not shown here.</strong>
      Each row is the newest build of that tool anybody has measured; the ones it replaced are
      still in the solve, still holding the graph together.</p>
      <p class="fine">A version is a competitor, not a label — a release that got faster is the
      thing being reported, so it gets its own row rather than being averaged with the release
      before it. See <a href="builds.html">every build measured</a> to compare two releases of one
      tool, or to pick a build per tool and read the ranking that follows from it.</p>
    </div>`
		: ""
}${
	result.buildSpread?.length
		? (
				() => {
					// The one fold this design keeps: rc.1, rc.2 and rc.3 are one node, because four
					// rc rows for one release is noise. Two candidates for a release can differ as
					// much as two releases do, so where the fold actually caught more than one build,
					// the page names them instead of presenting a single measured thing.
					const say = (v) =>
						`<code>${esc(displayTool(v.tool))} ${esc(v.build)}</code> on ${esc(v.versions.join(", "))} across ${plural(v.submissions, "run")}`;
					return `
    <div class="aside after">
      <p class="lead"><strong>A prerelease line rests on more than one candidate.</strong>
      Iterations of one prerelease are ranked as a single build, so this row is an rc series
      rather than a specific one.</p>
      <p class="fine">${result.buildSpread.map(say).join("; ")}. Two candidates for one release can
      differ as much as two releases do; if that matters here, the runs behind each are in the
      table below.</p>
    </div>`;
				}
			)()
	...
	result.unplaceable?.length
		? `
    <div class="aside after">
      <p class="lead"><strong>${plural(result.unplaceable.length, "measurement")} could not be placed.</strong>
      A measurement that does not record which build it measured has no node to occupy, so it is
      excluded rather than folded into one.</p>
      <p class="fine">${[...new Set(result.unplaceable.map((u) => displayTool(u.tool)))].map((t) => `<code>${esc(t)}</code>`).join(", ")} — a
      missing version is a defect in the submitting harness, and the fix is to re-measure rather
      than to assume.</p>
    </div>`
		: ""
}
...
${builds
	...
	const lede = multi.length
		? `${multi.map(([t]) => displayTool(t.tool)).join(", ")} ${multi.length === 1 ? "has" : "have"} been measured on more than one build, so ${multi.length === 1 ? "its releases can" : "their releases can"} be read against each other.`
...
<div class="page">
  ...
  <section class="hero">
    <div>
      ...
      <p class="hero-lede">A version is a competitor here, not a label: each build is its own node
      in the same solve, so a release and the release before it are ranked against each other
      rather than averaged together. ${esc(lede)}</p>
    </div>
  </section>
  ...
  <section class="lead-section">
    ...
    <p class="prose">Untick a build to take it off the chart. Nothing is re-solved when you do —
    every figure below comes from one solve over every edge, so two builds compare the same way
    whether or not a third is on screen.</p>
    <div class="scopes">
${picker}
    </div>
