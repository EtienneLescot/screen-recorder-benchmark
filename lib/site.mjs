...
<main>
  ...
  ${
		...
		c?.measurable
			? `<div class="note"><p class="lead"><strong>The machines do not fully agree.</strong> Where several measured the same pair, they differ by ${c.medianResidualPercent}% at the median.</p><p class="fine">The worst pair is ${c.maxResidualPercent}%${c.worst ? ` — <code>${esc(displayTool(c.worst.a))}</code> against <code>${esc(displayTool(c.worst.b))}</code> on ${esc(c.worst.machine)}` : ""}. Disagreement of this size is not error: it is the ranking depending on the hardware. Only pairs measured on more than one machine can be checked this way, which is why more submissions make the figure stronger rather than noisier.</p></div>`
			: `<div class="note"><p class="lead"><strong>Not cross-checked yet.</strong> Every ratio here rests on a single machine.</p><p class="fine">Every ratio here rests on a
      single machine, so nothing contradicts anything and no agreement figure is meaningful.
      Consistency becomes measurable once two submissions measure an overlapping pair.</p></div>`
	}

  ${
		result.versionSpread?.length
			? `<div class="note warn"><p class="lead"><strong>Mixed versions.</strong> A figure here rests on more than one build.</p><p class="fine">
